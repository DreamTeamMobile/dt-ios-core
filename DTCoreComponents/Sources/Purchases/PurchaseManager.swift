    ...
    PurchaseManagerProtocol
{
    ...
    func loadPurchasedProducts(
        validate: Bool,
        completion: ((_ valid: Bool, _ error: Error?) -> Void)?,
        sandbox: Bool = false
    ) {
        ...
        if let productIdentifiers = productIdentifiers {
            ...
            if validate {
                ...
                validateReceipt(sandbox) { [weak self] (valid, error) in
                    ...
                    os_log(
                        "8.Receipt is Valid. Checking local receipt...",
                        log: OSLog.default,
                        type: .info
                    )
                    self?.validateLocalReceipt { (active, error) in
                        ...
                        if active {
                            ...
                            os_log(
                                "10.BEWARE! Reciept is not Active!!!",
                                log: OSLog.default,
                                type: .info
                            )
                        }
    ...
    fileprivate func validateLocalReceipt(
        completion: @escaping (_ valid: Bool, _ error: Error?) -> Void
    ) {
        do {
            ...
            for productId in purchasedProductIdentifiers {
                if let _ = nonConsumableIdentifiers?.first(where: {
                    receipt.containsPurchase(ofProductIdentifier: $0)
                }) {
                    os_log(
                        "9.Local receipt has active non-consumable purchase",
                        log: OSLog.default,
                        type: .info
                    )
                    completion(true, nil)
                ...
                else if receipt.autoRenewablePurchases.contains(where: {
    ...
    fileprivate func validateReceipt(
        ...
        if let url = Bundle.main.appStoreReceiptURL, let r = try? Data(contentsOf: url) {
            ...
            do {
                ...
                os_log(
                    "5.validateReceipt: Caught error serializing response JSON",
                    log: OSLog.default,
                    type: .info
                )
                    ...
                    if status == 0 {
                            ...
                            (jsonResponse["pending_renewal_info"] as? [[String: AnyObject]])
                        {
                            let activeSubscription = array.first(where: { (dict) -> Bool in
                                ...
                                guard let expirationIntent = dict["expiration_intent"] as? String
                                else {
                                    return true
                                }
                                ...
                                let isInBillingRetryPeriod =
                                    dict["is_in_billing_retry_period"] as? String
                        ...
                        #if DEBUG
                            if let _ = Bundle.lookUp(
                                forResource: "StoreKitTestCertificate",
                                ofType: "cer"
                            ) {
     ...
     */
    public func checkPurchase(
        for productIdentifier: String
    ) -> (isPurchased: Bool, hasValidReceipt: Bool) {
    ...
    public func productsRequest(
        ...
        for skProduct in skProducts {
            print(
                "Found product: \(skProduct.productIdentifier) - \(skProduct.localizedTitle) - \(skProduct.price) - \(skProduct.priceLocale)"
            )
        }
    ...
    public func checkSubscription(withAppleCheck: Bool, completion: ((Bool, Error?) -> Void)?) {
        ...
        self.loadPurchasedProducts(
            validate: withAppleCheck,
            completion: { (isValid, error) in
